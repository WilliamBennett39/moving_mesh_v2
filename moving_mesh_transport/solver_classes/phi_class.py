...
class scalar_flux(object):
                ...
                for l in range(self.N_ang):
                    for j in range(self.M+1):
                        for k in range(self.Msigma + 1):
                            self.PV[i] += (self.sigma_s / self.sigma_t) * self.cs[l, space, k] * u[l,j] * self.ws[l] * self.AAA[i, j, k] 
                            self.PV[i] += (self.sigma_s / self.sigma_t) * self.cs_moment_1[l, space, k] * u[l,j] * self.ws[l] * self.AAA[i, j, k] * self.mus[l] 
    ...
    def get_coeffs(self, opacity_class):
        ...
        self.cs_moment_1 = opacity_class.cs_moment_1
